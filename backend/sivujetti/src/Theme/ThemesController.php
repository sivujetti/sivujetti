...
final class ThemesController {
    ...
    /**
     * PUT /api/themes/[i:themeId]/style-units-var-vals: Overwrites styleUnitVarVals
     * of theme $req->params->themeId with $req->body->varValsItems.
     *
     * @param \Pike\Request $req
     * @param \Pike\Response $res
     * @param \Pike\Db\FluentDb $db
    ...
    public function updateStyleVarValStyless(Request $req, Response $res, FluentDb $db, FileSystem $fs): void {
        ...
        $db->getDb()->runInTransaction(function () use ($req, $db, $fs) {
            ...
            $updatedAll = self::replaceLinesBetween(
                ...
                startLine: "/* -- unit-var-vals classes start -- */\n",
                endLine: "/* -- unit-var-vals classes end -- */\n"
            );

            // 2. Commit to db
            $db->update("\${p}themes")
                ->values((object) ["styleUnitVarVals" => JsonUtils::stringify($cleaned),
            ...
            // 4. Commit to disk
            $fs->write(SIVUJETTI_INDEX_PATH . "public/{$current->themeName}-temp.css", (
                "@charset \"utf-8\";\n" .
                "\n" .
    ...
    private function validateOverwriteVarValStylesInput(object $input): array {
        return Validation::makeObjectValidator()
            ->addRuleImpl("compactId", fn($value) =>
                is_string($value) &&
                strlen($value) <= strlen("j-svv-4294967295") &&
                Validation::isStringType(str_replace(["_", "-"], "", $value), "alnum")
            , "%s is not valid compact id")
            ->rule("varValsItems", "minLength", 1, "array")
            ->rule("varValsItems.*.id", "compactId")
            ->rule("varValsItems.*.styleUnitMetaId", "compactId")
            ->rule("varValsItems.*.values", "minLength", 1, "array")
